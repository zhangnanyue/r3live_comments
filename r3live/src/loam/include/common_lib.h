...
#define STD_VEC_FROM_EIGEN(mat)                                                \
  std::vector<decltype(mat)::Scalar>(mat.data(),                               \
...
static const Eigen::Vector3d Lidar_offset_to_IMU(0.04165, 0.02326,
                                                 -0.0284); // Avia
...
right_jacobian_of_rotion_matrix(const Eigen::Matrix<T, 3, 1> &omega) {
...
template <typename T = double>
Eigen::Matrix<T, 3, 3>
inverse_right_jacobian_of_rotion_matrix(const Eigen::Matrix<T, 3, 1> &omega) {
  // Barfoot, Timothy D, State estimation for robotics. Page 232-237
  Eigen::Matrix<T, 3, 3> res_mat_33;
  ...
  Eigen::Matrix<T, 3, 3> hat_a = vec_to_hat(a);
  res_mat_33 =
      (theta / 2) * (cot(theta / 2)) * Eigen::Matrix<T, 3, 3>::Identity() +
...
struct Camera_Lidar_queue {
  ...
  // sensor_msgs::PointCloudConstPtr>> *m_camera_frame_buf = nullptr;
  ...
  int camera_in(const double &in_time) {
    if (in_time < m_last_imu_time - m_sliding_window_tim) {
      std::cout << ANSI_COLOR_RED_BOLD
                << "Camera incoming frame too old, need to be drop!!!"
                << ANSI_COLOR_RESET << std::endl;
  ...
  // 1. 获取正在处理的图像数据时间戳
  // 2. 获取雷达数据buffer中最旧的时间戳
  // 3. 当雷达有数据，且最旧的雷达数据时间大于当前正在处理的图像时间戳
  ...
  void display_last_cam_LiDAR_time() {
    ...
    cout << std::setprecision(15) << "Camera time = " << cam_last_time
         << ", LiDAR last time =  " << lidar_last_time << endl;
  }
  ...
  bool if_lidar_can_process() {
    ...
    if (cam_last_time < 0 || lidar_last_time < 0) {
      // cout << "Cam_tim = " << cam_last_time << ", lidar_last_time = " <<
      // lidar_last_time << endl;
      return false;
    }
    ...
    if (lidar_last_time > cam_last_time) {
      ...
      // cout << "LiDAR can update, " << get_lidar_front_time() -
      // m_first_imu_time << " | " << get_camera_front_time() - m_first_imu_time
      // << endl; printf_line;
      return true;
    }
    return false;
...
{
  MeasureGroup() { this->lidar.reset(new PointCloudXYZINormal()); };
...
public:
  ...
  Eigen::Matrix3d rot_end; // [0-2] the estimated attitude (rotation matrix) at
                           // the end lidar point
  Eigen::Vector3d pos_end; // [3-5] the estimated position at the end lidar
                           // point (world frame)
  Eigen::Vector3d vel_end; // [6-8] the estimated velocity at the end lidar
                           // point (world frame)
  ...
  Eigen::Matrix3d rot_ext_i2c; // [18-20] Extrinsic between IMU frame to Camera
                               // frame on rotation.
  Eigen::Vector3d pos_ext_i2c; // [21-23] Extrinsic between IMU frame to Camera
                               // frame on position.
  double td_ext_i2c_delta;     // [24]    Extrinsic between IMU frame to Camera
                               // frame on position.
  ...
  StatesGroup
  operator+(const Eigen::Matrix<double, DIM_OF_STATES, 1> &state_add) {
    StatesGroup a = *this;
    // a.rot_end = this->rot_end * Sophus::SO3d::exp(vec_3(state_add(0, 0),
    // state_add(1, 0), state_add(2, 0) ) );
    ...
    // Ext camera w.r.t. IMU
    a.rot_ext_i2c =
        this->rot_ext_i2c * Exp(state_add(18), state_add(19), state_add(20));
