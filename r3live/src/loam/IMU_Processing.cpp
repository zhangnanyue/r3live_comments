...
ImuProcess::ImuProcess()
    : b_first_frame_(true), imu_need_init_(true), last_imu_(nullptr),
      start_timestamp_(-1) {
...
bool check_state(StatesGroup &state_inout) {
  ...
  for (int idx = 0; idx < 3; idx++) {
    if (fabs(state_inout.vel_end(idx)) > 10) {
      ...
      for (int i = 0; i < 10; i++) {
        cout << __FILE__ << ", " << __LINE__ << ", check_state fail !!!! "
             << state_inout.vel_end.transpose() << endl;
      }
...
void check_in_out_state(const StatesGroup &state_in, StatesGroup &state_inout) {
  if ((state_in.pos_end - state_inout.pos_end).norm() > 1.0) {
    scope_color(ANSI_COLOR_RED_BG);
    for (int i = 0; i < 10; i++) {
      cout << __FILE__ << ", " << __LINE__ << ", check_in_out_state fail !!!! "
...
// 1. 调用check_state()检查速度分量是否过大？
// 2. 初始化此时的旋转，位置，速度（使用lidar状态），初始化状态量的雅可比矩阵Fx,
// cov_w矩阵
// 3. 循环imu队列，进行预积分
//    3.1 求平均加速度和角速度，并减去bias（名义上的状态量）
//    3.2 检查时间，如果位于第一帧的imu时间小于上一次优化状态时间，则返回
//    3.3 计算dt,，主要是一个imu和后续imu的dt获取方式不同
//    3.4 计算FX，计算 Fw * Q * FW.trans，传播P
//    3.5 状态更新，包括：姿态，位置，速度，last_acc, last_angvel
// 4. 此时imu预积分的状态时间戳应该小于当前相机帧的时间戳，则
//    4.1 计算imu最后一帧到当前帧的时间差，继续传播，计算姿态，位置，速度
//    4.2 将last_update_time更新到最新一帧的时间戳
// 5.
// 调用check_in_out_state()检查预积分前后pos是否超过1m，超过认为无效，将传播后状态重置为传播前
...
ImuProcess::imu_preintegration(const StatesGroup &state_in,
  ...
  Eigen::Vector3d acc_imu(0, 0, 0), vel_imu(0, 0, 0), angvel_avr(0, 0, 0),
      acc_avr(0, 0, 0), pos_imu(0, 0, 0);
  ...
  for (std::deque<sensor_msgs::Imu::ConstPtr>::iterator it_imu = v_imu.begin();
    ...
    // Eigen::Matrix3d Jr_omega_dt =
    // right_jacobian_of_rotion_matrix<double>(angvel_avr*dt);
    ...
    cov_omega_diag = Eigen::Vector3d(COV_OMEGA_NOISE_DIAG, COV_OMEGA_NOISE_DIAG,
                                     COV_OMEGA_NOISE_DIAG)
                         .asDiagonal();
    cov_acc_diag = Eigen::Vector3d(COV_ACC_NOISE_DIAG, COV_ACC_NOISE_DIAG,
                                   COV_ACC_NOISE_DIAG)
                       .asDiagonal();
    cov_gyr_diag = Eigen::Vector3d(COV_GYRO_NOISE_DIAG, COV_GYRO_NOISE_DIAG,
                                   COV_GYRO_NOISE_DIAG)
                       .asDiagonal();
    // cov_w.block<3, 3>(0, 0) = cov_omega_diag * dt * dt;
    cov_w.block<3, 3>(0, 0) =
        Jr_omega_dt * cov_omega_diag * Jr_omega_dt * dt * dt;
    cov_w.block<3, 3>(3, 3) =
        R_imu * cov_gyr_diag * R_imu.transpose() * dt * dt;
    cov_w.block<3, 3>(6, 6) = cov_acc_diag * dt * dt;
    cov_w.block<3, 3>(9, 9).diagonal() =
        Eigen::Vector3d(COV_BIAS_GYRO_NOISE_DIAG, COV_BIAS_GYRO_NOISE_DIAG,
                        COV_BIAS_GYRO_NOISE_DIAG) *
        dt * dt; // bias gyro covariance
    cov_w.block<3, 3>(12, 12).diagonal() =
        Eigen::Vector3d(COV_BIAS_ACC_NOISE_DIAG, COV_BIAS_ACC_NOISE_DIAG,
                        COV_BIAS_ACC_NOISE_DIAG) *
        dt * dt; // bias acc covariance

    // cov_w.block<3, 3>(18, 18).diagonal() =
    // Eigen::Vector3d(COV_NOISE_EXT_I2C_R, COV_NOISE_EXT_I2C_R,
    // COV_NOISE_EXT_I2C_R) * dt * dt; // bias gyro covariance cov_w.block<3,
    // 3>(21, 21).diagonal() = Eigen::Vector3d(COV_NOISE_EXT_I2C_T,
    // COV_NOISE_EXT_I2C_T, COV_NOISE_EXT_I2C_T) * dt
    // * dt;  // bias acc covariance cov_w(24, 24) = COV_NOISE_EXT_I2C_Td * dt *
    // dt;
  ...
  // std::setprecision(2) <<(state_inout - StatesGroup()).transpose() << endl;
  /*** calculated the pos and attitude prediction at the frame-end ***/
  dt = end_pose_dt;

  state_inout.last_update_time = v_imu.back()->header.stamp.toSec() + dt;
  // cout << "Last update time = " <<  state_inout.last_update_time -
  // g_lidar_star_tim << endl;
  if (dt > 0.1) {
  ...
  // cout << "Preintegration State diff = " << std::setprecision(2) <<
  // (state_inout - state_in).head<15>().transpose()
  //      <<  endl;
...
void ImuProcess::lic_point_cloud_undistort(const MeasureGroup &meas,
  ...
  const double &pcl_end_time =
      pcl_beg_time + pcl_out.points.back().curvature / double(1000);
  /*std::cout << "[ IMU Process ]: Process lidar from " << pcl_beg_time -
     g_lidar_star_tim << " to " << pcl_end_time- g_lidar_star_tim << ", "
            << meas.imu.size() << " imu msgs from " << imu_beg_time-
     g_lidar_star_tim << " to " << imu_end_time- g_lidar_star_tim
            << ", last tim: " << state_inout.last_update_time- g_lidar_star_tim
     << std::endl;
  */
  /*** Initialize IMU pose ***/
  IMU_pose.clear();
  ...
  IMU_pose.push_back(set_pose6d(0.0, acc_s_last, angvel_last,
...
#ifdef DEBUG_PRINT
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<"
// "<<acc_avr.transpose()<<std::endl;
#endif
    dt = tail->header.stamp.toSec() - head->header.stamp.toSec();
    /* covariance propagation */
...
// "<<acc_avr.transpose()<<std::endl;
    ...
    // std::cout<<"acc "<<acc_imu.transpose()<<"vel "<<acc_imu.transpose()<<"vel
    // "<<pos_imu.transpose()<<std::endl;
    IMU_pose.push_back(
...
#ifdef DEBUG_PRINT
  std::cout << "[ IMU Process ]: vel " << state_inout.vel_end.transpose()
            << " pos " << state_inout.pos_end.transpose() << " ba"
            << state_inout.bias_a.transpose() << " bg "
            << state_inout.bias_g.transpose() << std::endl;
  std::cout << "propagated cov: " << state_inout.cov.diagonal().transpose()
            << std::endl;
#endif
  ...
  for (auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp--) {
    ...
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
      ...
      Eigen::Vector3d P_compensate =
          state_inout.rot_end.transpose() * (R_i * P_i + T_ei);
...
void ImuProcess::Process(const MeasureGroup &meas, StatesGroup &stat,
  ...
  if (imu_need_init_) {
    ...
    if (init_iter_num > MAX_INI_COUNT) {
      ...
      // std::cout<<"mean acc: "<<mean_acc<<" acc measures in word
      // frame:"<<state.rot_end.transpose()*mean_acc<<std::endl;
      ROS_INFO("IMU Initials: Gravity: %.4f %.4f %.4f; state.bias_g: %.4f %.4f "
               "%.4f; acc covarience: %.8f %.8f %.8f; gry covarience: %.8f "
               "%.8f %.8f",
               stat.gravity[0], stat.gravity[1], stat.gravity[2],
               stat.bias_g[0], stat.bias_g[1], stat.bias_g[2], cov_acc[0],
